...
class FeedRankingRepository:
    ...
    # ========== CONTENT AFFINITY QUERIES (ASYNC) ==========

    async def get_user_primary_category_async(self, db: AsyncSession, user_id: int, gym_id: int) -> Optional[str]:
        """Obtiene la categoría fitness primaria del usuario (async)."""
        query = text("""
            SELECT c.category_enum
            FROM class_participation cp
            JOIN class_session cs ON cp.session_id = cs.id
    ...
    async def get_user_category_distribution_async(self, db: AsyncSession, user_id: int, gym_id: int) -> Dict[str, float]:
        """Obtiene la distribución de categorías del usuario (async)."""
        query = text("""
            WITH category_counts AS (
                SELECT
                    c.category_enum,
    ...
    async def get_post_categories_async(self, db: AsyncSession, post_id: int) -> List[str]:
        """Obtiene categorías/tags del post (async)."""
        query = text("""
            SELECT tag_value
            FROM post_tags
            WHERE post_id = :post_id
    ...
    # ========== SOCIAL AFFINITY QUERIES (ASYNC) ==========

    async def get_user_relationship_type_async(
        self,
        db: AsyncSession,
        user_id: int,
        author_id: int,
        ...
        """Determina el tipo de relación entre usuario y autor (async)."""
        # 1. Verificar si author es trainer del user
        query_trainer = text("""
            SELECT 1 FROM trainermemberrelationship
            WHERE trainer_id = :author_id
    ...
    async def get_past_interactions_count_async(
        self,
        db: AsyncSession,
        user_id: int,
        author_id: int,
        ...
        """Cuenta interacciones previas del usuario con posts del autor (async)."""
        query = text("""
            SELECT COUNT(*) as interaction_count
            FROM (
                SELECT pl.created_at
    ...
    # ========== PAST ENGAGEMENT QUERIES (ASYNC) ==========

    async def get_user_engagement_patterns_async(
        self,
        db: AsyncSession,
        user_id: int,
        gym_id: int,
        ...
        """Analiza patrones de engagement del usuario (async)."""
        query = text("""
            WITH user_likes AS (
                SELECT
                    p.id as post_id,
    ...
    # ========== TIMING QUERIES (ASYNC) ==========

    async def get_user_active_hours_async(
        self,
        db: AsyncSession,
        user_id: int,
        gym_id: int,
        ...
        """Detecta las horas del día en las que el usuario es más activo (async)."""
        query = text("""
            WITH user_activity AS (
                SELECT EXTRACT(HOUR FROM pl.created_at)::int as hour
                FROM post_likes pl
    ...
    # ========== POPULARITY QUERIES (ASYNC) ==========

    async def get_post_engagement_metrics_async(
        self,
        db: AsyncSession,
        post_id: int,
        gym_id: int
    ) -> Dict[str, any]:
        """Obtiene métricas de engagement del post (async)."""
        query = text("""
            SELECT
                p.like_count as likes,
                p.comment_count as comments,
    ...
    async def get_gym_engagement_percentiles_async(
        self,
        db: AsyncSession,
        gym_id: int,
        hours_lookback: int = 24
    ) -> Dict[str, float]:
        """Calcula percentiles de engagement para posts recientes del gym (async)."""
        query = text("""
            WITH recent_posts AS (
                SELECT
                    p.id,
    ...
    # ========== UTILITY QUERIES (ASYNC) ==========

    async def get_viewed_post_ids_async(self, db: AsyncSession, user_id: int, gym_id: int, days: int = 7) -> List[int]:
        """Obtiene IDs de posts ya vistos por el usuario (async)."""
        query = text("""
            SELECT DISTINCT post_id
            FROM post_views
            WHERE user_id = :user_id
