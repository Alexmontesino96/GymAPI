...
class SurveyRepository:
    ...
    async def _create_question_async(
        ...
        """Helper para crear una pregunta con opciones (async)"""
        question_dict = question_data.dict(exclude={'choices'})
        question_dict['order'] = order
        question = SurveyQuestion(
            survey_id=survey_id,
    ...
    async def create_survey_async(
        ...
        """Crear una nueva encuesta con preguntas (async)"""
        try:
            # Crear encuesta principal
            survey_data = survey_in.dict(exclude={'questions'})
            survey = Survey(
            ...
            # Crear preguntas y opciones
            for idx, question_data in enumerate(survey_in.questions or []):
                question = await self._create_question_async(
                    db=db,
                    survey_id=survey.id,
                    question_data=question_data,
                    order=idx
    ...
    async def get_survey_async(
        ...
        """Obtener una encuesta por ID (async)"""
    ...
    async def get_surveys_async(
        ...
        """Obtener lista de encuestas con filtros (async)"""
    ...
    async def get_surveys_with_response_count_async(
        self,
        db: AsyncSession,
        gym_id: int,
        skip: int = 0,
        ...
        """Obtener encuestas del gimnasio con conteo de respuestas (async)"""
    ...
    async def get_active_surveys_async(
        self,
        db: AsyncSession,
        gym_id: int,
        user_id: Optional[int] = None
    ) -> List[Survey]:
        """Obtener encuestas activas disponibles para un usuario (async)"""
        now = datetime.utcnow()
    ...
    async def update_survey_async(
        ...
        """Actualizar una encuesta (async)"""
        survey = await self.get_survey_async(db, survey_id, gym_id)
    ...
    async def delete_survey_async(
        ...
        """Eliminar una encuesta (solo si está en borrador) (async)"""
        survey = await self.get_survey_async(db, survey_id, gym_id)
    ...
    async def close_survey_async(
        self,
        db: AsyncSession,
        survey_id: int,
        gym_id: int
    ) -> Optional[Survey]:
        """Cerrar una encuesta (async)"""
        survey = await self.get_survey_async(db, survey_id, gym_id)
    ...
    async def _validate_and_create_answers_async(
        self,
        db: AsyncSession,
        response_id: int,
        answers: List[AnswerCreate],
        ...
        """Validar y crear respuestas individuales (async)"""
        # Crear diccionario de preguntas para validación rápida
        questions_dict = {q.id: q for q in survey.questions}
    ...
    async def create_response_async(
        ...
        """Crear una respuesta completa a una encuesta (async)"""
        # Verificar que la encuesta existe y está activa
        survey = await self.get_survey_async(db, response_in.survey_id, gym_id)
        ...
        try:
            ...
            # Validar y crear answers
            await self._validate_and_create_answers_async(
                db=db,
                response_id=response.id,
                answers=response_in.answers,
                survey=survey
    ...
    async def get_survey_responses_async(
        self,
        db: AsyncSession,
        survey_id: int,
        gym_id: int,
        ...
        """Obtener respuestas de una encuesta (async)"""
    ...
    async def get_user_responses_async(
        self,
        db: AsyncSession,
        user_id: int,
        gym_id: int,
        ...
        """Obtener respuestas de un usuario (async)"""
        stmt = select(SurveyResponse).where(
            and_(
                SurveyResponse.user_id == user_id,
                SurveyResponse.gym_id == gym_id
    ...
    async def create_template_async(
        ...
        """Crear una plantilla de encuesta (async)"""
        template = SurveyTemplate(
            **template_in.dict(),
            gym_id=gym_id
        )
    ...
    async def get_templates_async(
        ...
        """Obtener plantillas disponibles (async)"""
    ...
    async def create_survey_from_template_async(
        self,
        db: AsyncSession,
        template_id: int,
        title: str,
        ...
        """Crear una encuesta desde una plantilla (async)"""
