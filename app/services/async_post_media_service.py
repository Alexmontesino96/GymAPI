"""
AsyncPostMediaService - Servicio async para manejar archivos de posts.

Este módulo extiende AsyncStorageService para soportar galerías de hasta 10 archivos
con upload paralelo y generación automática de thumbnails.
...
logger = logging.getLogger("async_post_media_service")
settings = get_settings()


class AsyncPostMediaService(AsyncStorageService):
    """
    Servicio async para manejar media de posts (imágenes y videos con soporte de galería).

    Todos los métodos son async y utilizan AsyncSession.

    Hereda de AsyncStorageService y añade:
    - Upload de media individual y en galería (hasta 10 archivos)
    - Upload paralelo con asyncio.gather
    - Generación automática de thumbnails 800x800
    - Validación de tipos de archivo
    - Extracción de dimensiones de imágenes

    Métodos principales:
    - upload_post_media() - Sube un archivo individual
    - upload_gallery() - Sube múltiples archivos en paralelo
    - delete_post_media() - Elimina media del storage
    """

    def __init__(self):
        """
        Inicializa el servicio con configuración de posts.

        Note:
            Límites:
            - Imágenes: 10MB
            - Videos: 100MB
            - Máximo 10 archivos por galería
        """
        super().__init__()
    ...
    async def upload_post_media(
        ...
        Returns:
            ...
            - display_order, width, height (para imágenes)

        Raises:
            HTTPException: Si el tipo de archivo no es válido o excede tamaño máximo

        Note:
            Genera thumbnails automáticamente para imágenes >800px.
        """
    ...
    async def upload_gallery(
        ...
        Raises:
            HTTPException: Si no hay archivos, excede el máximo, o falla upload

        Note:
            - Upload paralelo con asyncio.gather
            - Si un archivo falla, limpia todos los ya subidos
            - Auto-detección de media_type por content_type y extensión
        """
        if not files:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def delete_post_media(self, media_url: str):
        ...
        Note:
            No lanza excepciones, solo registra errores en log.
            Formato URL esperado: https://.../storage/v1/object/public/{bucket}/{path}
        """
        try:
    ...
    async def _generate_image_thumbnail(
        ...
        Note:
            - Tamaño thumbnail: 800x800 manteniendo aspect ratio
...
_async_post_media_service_instance = None


def get_async_post_media_service() -> AsyncPostMediaService:
    """
    Obtiene la instancia singleton del AsyncPostMediaService.

    Returns:
        Instancia única de AsyncPostMediaService
    """
    global _async_post_media_service_instance
    if _async_post_media_service_instance is None:
        _async_post_media_service_instance = AsyncPostMediaService()
    return _async_post_media_service_instance
