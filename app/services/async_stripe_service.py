...
logger = logging.getLogger("async_stripe_service")
...
class AsyncStripeService:
    def __init__(self, membership_service: AsyncMembershipService):
    ...
    async def create_checkout_session(
        ...
        try:
            ...
            stripe_customer_id = await async_stripe_connect_service.get_or_create_customer_for_user_gym(
                db, int(user_id), gym_id
            )

            # Obtener cuenta de Stripe del gym
            gym_account = await async_stripe_connect_service.get_gym_stripe_account(db, gym_id)
            if not gym_account:
    ...
    async def handle_successful_payment(
        ...
        try:
            ...
            stripe_profile = await async_stripe_connect_service.get_user_stripe_profile(db, int(user_id), gym_id)
            ...
            if stripe_subscription_id and stripe_profile:
                ...
                from app.services.async_stripe_connect_service import async_stripe_connect_service
                await async_stripe_connect_service.update_subscription_for_user_gym(
    ...
    async def _handle_checkout_completed(self, session: Dict[str, Any]) -> None:
        ...
        try:
            ...
            async for db in get_async_db():
                try:
                    # Verificar que el customer existe en nuestra tabla de vinculación
                    stripe_profile = await async_stripe_connect_service.get_user_stripe_profile(db, int(user_id), int(gym_id))
                    if not stripe_profile:
                        logger.warning(f"No se encontró perfil de Stripe para user {user_id} en gym {gym_id}")
                        # Crear el perfil si no existe (caso edge)
                        await async_stripe_connect_service.get_or_create_customer_for_user_gym(
    ...
    async def create_stripe_product_for_plan(
        ...
        try:
            ...
            gym_account = await async_stripe_connect_service.get_gym_stripe_account(db, plan.gym_id)
            if not gym_account:
...
# 🆕 MÉTODOS DE NOTIFICACIÓN Y ALERTAS
    ...
    async def _notify_payment_overdue(self, user_gym) -> None:
        ...
        try:
            ...
            async for db in get_async_db():
                try:
                    result = await db.execute(select(User).where(User.id == user_gym.user_id))
                    user = result.scalar_one_or_none()
                    if user and user.onesignal_player_id:
                        await async_notification_service.send_notification(
        ...
        try:
            ...
            async for db in get_async_db():
                try:
                    ...
                    result = await db.execute(
                        select(User).join(UserGym).where(
        ...
        try:
            ...
            async for db in get_async_db():
                try:
                    ...
                    query = select(User).join(UserGym).where(
                        UserGym.role.in_([GymRoleType.OWNER, GymRoleType.ADMIN])
                    )

                    if gym_id:
                        query = query.where(UserGym.gym_id == int(gym_id))
...
_async_stripe_service_instance = None

def get_async_stripe_service():
    """
    Obtener la instancia del servicio Stripe async.
    La instancia se crea de forma lazy para evitar problemas de importación circular.
    """
    global _async_stripe_service_instance
    if _async_stripe_service_instance is None:
        # Importación lazy para evitar circular imports
        from app.services.async_membership import async_membership_service
        _async_stripe_service_instance = AsyncStripeService(async_membership_service)
    return _async_stripe_service_instance

# Función para obtener la instancia de forma lazy
def _get_async_stripe_service_lazy():
    """Función interna para lazy loading"""
    return get_async_stripe_service()

# Crear un objeto proxy que se inicializa solo cuando se accede
class AsyncStripeServiceProxy:
    def __getattr__(self, name):
        return getattr(get_async_stripe_service(), name)

# Exportar la instancia proxy para compatibilidad con importaciones existentes
async_stripe_service = AsyncStripeServiceProxy()
