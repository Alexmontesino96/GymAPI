"""
AsyncActivityAggregator - Servicio async para agregación de eventos de actividad.

Este módulo procesa eventos del sistema (check-ins, logros, récords) y los
...
logger = logging.getLogger("async_activity_aggregator")


class AsyncActivityAggregator:
    """
    Servicio async para agregar eventos del sistema en estadísticas anónimas.

    Todos los métodos son async y utilizan AsyncSession cuando acceden a BD.

    Características:
    - Procesa eventos individuales (check-ins, logros, récords)
    - Convierte eventos en métricas agregadas anónimas
    - Publica actividades en feed sin exponer identidades
    - Rankings anónimos basados en valores
    - Mensajes motivacionales según actividad

    Métodos principales:
    - on_class_checkin() - Procesa check-in a clase
    - on_achievement_unlocked() - Procesa logro desbloqueado
    - on_streak_milestone() - Procesa hito de racha
    - on_personal_record() - Procesa récord personal
    - calculate_hourly_summary() - Resumen cada hora
    - update_daily_rankings() - Rankings diarios anónimos
    """
    ...
    def __init__(self, feed_service: ActivityFeedService, db: Optional[AsyncSession] = None):
        """
        Inicializa el agregador async.

        Args:
            feed_service: Servicio de Activity Feed
            db: Sesión async de base de datos (opcional para queries agregadas)

        Note:
            - db es opcional: métodos de Redis funcionan sin él
            - update_daily_rankings() requiere db
        """
    ...
    async def on_class_checkin(self, event: Dict):
        ...
        Note:
            - Incrementa contadores de clase específica y totales
            - Publica actividad si alcanza umbral (cada 5 check-ins)
            - TTL: 5 minutos para contadores en tiempo real
        """
        gym_id = event["gym_id"]
    ...
    async def on_achievement_unlocked(self, event: Dict):
        ...
        Note:
            - Estadísticas completamente anónimas
            - Publica cada 3 logros acumulados en el día
        """
        gym_id = event["gym_id"]
    ...
    async def on_streak_milestone(self, event: Dict):
        ...
        Note:
            - Solo procesa hitos importantes: 7, 14, 21, 30, 60, 90, 180, 365 días
            - Publica automáticamente cada hito alcanzado
        """
        gym_id = event["gym_id"]
    ...
    async def on_personal_record(self, event: Dict):
        ...
        Note:
            - Estadísticas agregadas sin exponer quién rompió el récord
            - Publica cada 3 récords personales en el día
        """
        gym_id = event["gym_id"]
    ...
    async def on_goal_completed(self, event: Dict):
        ...
        Note:
            - Publica cada 5 metas completadas en el día
        """
        gym_id = event["gym_id"]
    ...
    async def on_class_completed(self, event: Dict):
        ...
        Note:
            - Actualiza horas totales entrenadas (aproximado)
            - Publica si la clase tuvo 15+ participantes
        """
        gym_id = event["gym_id"]
    ...
    async def calculate_hourly_summary(self, gym_id: int):
        ...
        Note:
            - Genera mensajes motivacionales basados en actividad
            - Publica máximo 3 mensajes destacados
            - Mantiene últimos 100 items en feed (LTRIM)
        """
    ...
    async def update_daily_rankings(self, gym_id: int):
        ...
        Note:
            - Requiere self.db (AsyncSession)
            - Solo publica valores, no nombres de usuarios
            - Rankings: consistencia (rachas) y asistencia del día
        """
        if not self.db:
    ...
    async def generate_motivational_burst(self, gym_id: int):
        ...
        Note:
            - Detecta hora pico (>20 usuarios activos)
            - Detecta entrenamiento grupal (3+ grupos)
        """
    ...
    async def _gather_hourly_stats(self, gym_id: int) -> Dict:
        ...
        Returns:
            ...
            - total_attendance, new_prs, goals_completed
            - achievements, total_hours, classes_completed
        """
        stats = {}
    ...
    async def _gather_current_stats(self, gym_id: int) -> Dict:
        ...
        Returns:
            ...
            - is_peak_time: True si >20 usuarios
            - group_training: número de grupos activos
            - popular_classes: lista de clases populares
        """
        stats = {
            "active_users": 0,
            "is_peak_time": False,
    ...
    async def process_batch_events(self, events: List[Dict]):
        ...
        Note:
            - Procesa cada evento con su handler específico
            - Maneja errores individualmente sin detener el batch
            - Handlers disponibles: class_checkin, achievement_unlocked,
              streak_milestone, personal_record, goal_completed, class_completed
        """
        for event in events:
